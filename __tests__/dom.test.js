...
const { queryByRole, queryByText, fireEvent} = require("@testing-library/dom");
const { calculateCost, getTotalCost,applyDiscount, applyCoupon } = require("./index.js");
...
describe("The document", () => {
  ...
  // TODO: Complete this test case so it is testing the described behavior.  This is throwing an error still.
  
  test("displays the correct cost for the user's input values after clicking the calculate button", () => {
    const price = queryByRole(document, "spinbutton", { name: /price/i });
    const subscribed = queryByRole(document, "checkbox", { name: /subscribed/i });
    const calculateButton = queryByRole(document, "button", { name: /calculate/i });
    ...
    const expectedCost = applyCoupon(costAfterSubscription, hasCoupon).toFixed(2);

    const displayedCost = calculateCost();
  
    expect(displayedCost).toBe(`$${expectedCost.toFixed(2)}`);
  });
});
